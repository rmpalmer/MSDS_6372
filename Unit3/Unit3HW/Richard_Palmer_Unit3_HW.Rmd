...
Define the contrasts that we are interested in.
```{r}
...
The residuals do appear normally distributed.  The spread appears to be wider at fitted 
values near 300, so the assumption of constant variance may be violated.
...
significant, so the association betwween strata and iriduam level is
...
Look again at the aov model.
```{r}
summary(iridium.fit)
```

From this, we conclude that DeptCat (p value 7.3e-5) and Strata (p value .00325)
are both statistically significant explanatory variables, but again, that the 
interaction is not significant.

Do a linear model separately for depth 
```{r}
iridium.fit_lm_depth <- lm(Iridium ~ DepthCat,data=ex1317)
summary(iridium.fit_lm_depth)

```

From this, it appears that only Depth Category 3 is
has a statistically significant difference from other 
levels.

Use TukeyHSD to examine the same question.

```{r}
TukeyHSD(iridium.fit,"DepthCat")
```

Looking at all the pairwise differences, the only
pairs that have statistically signifcant differences
involve a difference with Depth Category 3.

We conclude that only Depth Category 3 has a 
statistically significant influence on iridium level.
