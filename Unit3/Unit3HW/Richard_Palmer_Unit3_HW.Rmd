...
(re)Examine the ANOVA results
```{r}
Anova(iridium.fit,type=3)
```

From this, we conclude that only DepthCat has a statistically
significant contribution to the model.

So, use Tukey HSD to determine which levels of the DepthCat factor
have a statistically significant contribution.

```{r}
TukeyHSD(iridium.fit,"DepthCat")
```

From this, the only pairwise comparisons that are significant
are the ones involving DepthCat 3.  Thus, the only statistically
signficant predictor in the model of iridium is DepthCat equal to
three, or not equal to 3.